...
public class Main {
    ...
    public static void displayUsers() {
        QuerySQL.runQueryAndGetResults(
            "SELECT "+
                "users.id, users.login, users.password, roles.role, "+
                "users.first_name, users.last_name, users.date_of_birth, "+
                "sex.type, users.passport, "+
                "countries.country, cities.city, streets.street, addresses.building, addresses.flat,"+
                "driver_licenses.serial, dl_categories.category, driver_licenses.dl_issued, driver_licenses.dl_expire "+
            "FROM users "+
                "INNER JOIN roles ON users.role_id=roles.id "+
                "INNER JOIN sex ON users.sex_id=sex.id "+
                "INNER JOIN addresses ON users.address_id=addresses.id "+
                "INNER JOIN streets ON addresses.street_id=streets.id "+
                "INNER JOIN cities ON streets.city_id=cities.id "+
                "INNER JOIN countries ON cities.country_id=countries.id "+
                "INNER JOIN driver_licenses ON users.driver_license=driver_licenses.serial "+
                "INNER JOIN dl_categories ON driver_licenses.dl_category_id=dl_categories.id "+
            ";");
}
    ...
    private static String[] getAllInsertSQLs() {
        return new String[] {
                ...
                "INSERT INTO rents (id, car_id, user_id, start_date, end_date, cost, paid) VALUES\n" +
                        "(1, 1, 1, '2016-04-04', '2016-04-08', 0, 1),\n" +
                        "(2, 1, 2, '2016-02-08', '2016-02-24', 0, 1),\n" +
                        "(3, 3, 3, '2016-06-01', '2016-07-01', 0, 1),\n" +
                        "(4, 1, 4, '2016-03-07', '2016-03-09', 0, 1),\n" +
                        "(5, 4, 5, '2016-10-02', '2016-10-04', 0, 1),\n" +
                        "(6, 3, 5, '2016-10-10', '2016-10-12', 0, 1);",
                ...
                "INSERT INTO streets (id, city_id, street) VALUES\n" +
                        ...
                        "(4, 4, 'ул. Строителей'),\n" +
                        "(5, 4, 'ул. Садовая');",
                "INSERT INTO users (id, login, password, role_id, first_name, last_name, middle_name, date_of_birth, sex_id, passport, address_id, driver_license) VALUES\n" +
                        "(1, 'ivan1971', 'ivan1971pass', 2, 'Иванченко', 'Юрий', NULL, '1971-12-15', 1, 'AB123456', 1, 'AA123123'),\n" +
                        "(2, 'hannapavlova', 'pavlovahanna', 2, 'Анна', 'Павлова', NULL, '1980-01-11', 2, 'AB9876543', 2, 'BB987987'),\n" +
                        "(3, 'root', 'pass', 2, 'Сергей', 'Волков', NULL, '1979-03-10', 3, 'BM123456', 3, 'CC123123'),\n" +
                        "(4, 'rediska', 'olga1981', 2, 'Ольга', 'Полтавская', NULL, '1981-01-11', 4, 'BM9876543', 4, 'DD987987'),\n" +
                        "(5, 'fiona', 'password', 2, 'Фиона', 'Борщевская', NULL, '1962-04-04', 2, 'HB9876543', 5, 'EE987987'),\n" +
                        "(6, 'korol', 'korolyov', 1, 'Андрей', 'Королев', NULL, '1981-02-15', 1, 'HB123456', 6, 'FF987987');",
        };
    }
